...
class ViT_lora_fft(VisionTransformer):
    ...
    def forward(self, x, task_id, register_blk=-1, get_feat=False):
        ...
        for i, blk in enumerate(self.blocks):
            x = blk(x, task_id, register_blk==i, get_feat=get_feat)
...
class SiNet(nn.Module):
    ...
    def forward(self, image, get_feat=False, fc_only=False):
        if fc_only:
            fc_outs = []
