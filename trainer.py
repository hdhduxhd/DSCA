...
def _train(args):
    ...
    for task in range(data_manager.nb_tasks):
        ...
        time_start = time.time()
        cnn_accy, cnn_accy_with_task, nme_accy, cnn_accy_task, ridge_accy = model.eval_task()
        time_end = time.time()
        logging.info('Time:{}'.format(time_end - time_start))
        # raise Exception
        model.after_task()
        ...
        if ridge_accy is not None:
            logging.info('RIDGE: {}'.format(ridge_accy['grouped']))
            ridge_curve['top1'].append(ridge_accy['top1'])
        cnn_curve['top1'].append(cnn_accy['top1'])
        cnn_curve_with_task['top1'].append(cnn_accy_with_task['top1'])
        cnn_curve_task['top1'].append(cnn_accy_task)
        logging.info('CNN top1 curve: {}'.format(cnn_curve['top1']))
        if ridge_accy is not None:
            logging.info('RIDGE top1 curve: {}'.format(ridge_curve['top1']))
